...
setwd("C:/Users/rnremie0411/Documents/Corrplots")

# Step 2: Read and Import Data from the Excel File
file_path <- "C:/Users/rnremie0411/Documents/Rdata/Rainfall_MM_60_SWC_45_Loggers_daily_SUM.xlsx"
...
compute_and_plot_ccf <- function(shifted_data_list, logger_ts, antecedent_periods, data_type) {
  ...
  for (period_index in 1:length(antecedent_periods)) {
    ...
    # Initialize variables to store the overall min and max for the y-axis range
    overall_min_acf <- Inf
    overall_max_acf <- -Inf
    ...
    for (i in 1:ncol(logger_ts)) {
      ...
      # Update overall_min_acf and overall_max_acf to set consistent axis limits
      overall_min_acf <- min(overall_min_acf, min(ccf_data$acf))
      overall_max_acf <- max(overall_max_acf, max(ccf_data$acf))
      
      # Find the lag with the highest correlation (maximum r)
      ...
      ccf_plot <- ggplot(ccf_data, aes(x = lag, y = acf)) +
        ...
        # Plot the red asterisk for the max correlation (r)
        annotate("point", x = max_lag, y = max_acf, color = "red", size = 3, shape = 8) +
        
        ## Reposition the max r value text within the visible area
        annotate("text", x = -10, y = overall_max_acf + 0.1, 
                 label = paste("r =", round(max_acf, 2)), 
                 color = "black", size = 3, hjust = 1) +  # Adjusted x and y for text
        
        ## Position the (r) = text just to the left of the asterisk
        #annotate("text", x = max_lag - 1, y = max_acf + 0.05,  # Adjust x to be a little left of the asterisk
                 #label = paste("r =", round(max_acf, 2)), 
                 #color = "black", size = 3, hjust = 1) +  # Adjusted for desired placement
        
        ## Position the (r) = text just to the right of the asterisk
        #annotate("text", x = max_lag + 1, y = max_acf + 0.05,  # Adjust x to be a little right of the asterisk
                 #label = paste("r =", round(max_acf, 2)), 
                 #color = "black", size = 3, hjust = 1) +  # Adjusted for desired placement
        
        
        # Set the x-axis limit from -20 to 0 and the y-axis limit based on the min/max acf values
        coord_cartesian(xlim = c(-22, 0), ylim = c(overall_min_acf - 0.1, overall_max_acf + 0.1)) # Constrained x-axis from -20 to 0
    ...
    ggsave(paste0("Asterix_crop", data_type, "_", antecedent_periods[period_index], "_days.png"),
